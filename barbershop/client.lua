...
Citizen.CreateThread(function()
	for i = 1, #Config.ShopsBarber, 1 do
		local blip = AddBlipForCoord(Config.ShopsBarber[i])
...
Citizen.CreateThread(function()
	while true do
		...
		for i = 1, #Config.ShopsBarber, 1 do
			if #(plyCoords - Config.ShopsBarber[i]) < Config.DrawDistance then
				DrawMarker(29, Config.ShopsBarber[i], vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(0.5, 0.5, 0.5), 0, 255, 0, 100, false, false, 2, true, nil, nil, false)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		for i = 1, #Config.ShopsBarber, 1 do
			if #(plyCoords - Config.ShopsBarber[i]) < Config.MarkerSize.x then
				isInMarker = true
				currentZone = k
			end
		end
