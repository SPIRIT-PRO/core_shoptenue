	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'Tattoos_menu', {
		...
		if (data.current.value ~= nil) then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'Tattoos_Categories_menu', {
				...
				_TriggerServerEvent("tattoos:save", currentTattoos, data2.current.price, {collection = data.current.value, texture = data2.current.value})
...
AddEventHandler("skinchanger:loadSkin", function(skin)
	if not firstLoad then
		Citizen.CreateThread(function()
			...
			Citizen.Wait(75)
			_TriggerServerEvent("tattoos:GetPlayerTattoos_s")
		end)
